...
A script can bring in two things from outside itself: another script, with
...
file makes it a library: `lib/waves.axis` is an ordinary script, which graphs
...
- **Config.** An imported file's `config` applies too, with the importing
  script's settings winning wherever the two disagree.
- **The ticker.** The importing script's ticker replaces an imported one.
...
for that reason: a script may import both without the constant being defined
twice.
...
The preview watches everything a script imports, so saving any file the graph
is built from reloads it. A change made in the preview to something an import
brought in is not written back, since only the script the preview was opened
on is edited.
...
```
image "./photos/beach.jpg"             // a file, beside the script
image "https://example.com/beach.jpg"  // a URL, fetched by Desmos
image "data:image/png;base64,iVBOR…"   // the picture itself
```

A **file** is named the way an import names one: relative to the script, or
...
machine the script was written on, and a graph has to carry its pictures with
...
script would gain the whole picture in base64.
