...
hands back a `sourceMap` saying which lines produced which expression, and
`writeBackGraph` rewrites a statement over exactly those lines - so a new
statement form has to record where it came from, or a change made to it in a
graph lands on the wrong line. The three layout passes carry the span through;
`packages/compiler/test/writeback.test.mts` is what notices when one does not.
...
- **A property missing from a state is not a property that was removed.** The
  ...
  that slider deletes `max: 10` from somebody's file. `writeBackGraph` does;
  the harness test for it is the only thing that could have caught it.
